 ...
 */
import { v4 } from './meta/pkg/uuid.v8.3.2.js';
...
export class Client {
  ...
  enable(messageHandler) {
    const listener = function (e) {
      if (e.origin === this.swmOrigin && e.data) {
        ...
        messageHandler(e.data, this.inflight.resolve, this.inflight.reject);
        if (this.inflight.resolve) {
          // Default resolution: resolve the inflight promise with the response.
          this.inflight.resolve(e.data);
        }
      } else if (this.inflight.reject) {
        this.inflight.reject('mismatched origin');
      }
    }.bind(this);
  ...
  async getMessageResponse(messageType, payload = {}) {
    this.inflight.promise = new Promise(function (resolve, reject) {
