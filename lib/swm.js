...
export class Client {
  /**
   * Maps the registered postMessage targetOrigins to the associated window
   * objects.
   */
  ...
  enable(messageHandler) {
    ...
    Client.handlers.set(this.swmOrigin, this.window);
    this.window.addEventListener('message', listener, false);
    this.unsubscribe = () => {
      handlers.delete(this.swmOrigin);
...
export function checkTargetOrigin(targetOrigin) {
  ...
  if (ancestors.length === 0) {
    console.log('window is not embedded, limiting to sending responses only.');
  } else if (!ancestors.contains(targetOrigin)) {
    console.warn(
...
export function checkIsListening(window) {
  ...
  if (![...Client.handlers.values()].includes(window)) {
